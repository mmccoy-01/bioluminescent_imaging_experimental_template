...
inj_date <- as.Date("2024-02-10", format = "%Y-%m-%d")

trt_date <- as.Date("2024-02-23", format = "%Y-%m-%d")

eos_date <- as.Date("2024-03-23", format = "%Y-%m-%d")

engraftment_imaging_date <- as.Date("2024-02-22", format = "%Y-%m-%d")

dates <- as.Date(c("2024-02-10", "2024-02-16", "2024-02-22", "2024-03-01", "2024-03-08", "2024-03-15", "2024-03-22", format = "%Y-%m-%d"))

best_seed <- 4347 # Found after running 'Treatment Assignment' chunk
number_of_groups <- 4
mice_per_group <- 5
filtered_mice <- c(1, 3:6, 9:13, 15:18, 20:25) # excluding 2, 7, 8, 14, 19
labels <- c("0" = "Non-study", "1" = "Treatment A", "2" = "Treatment B", "3" = "Treatment C", "4" = "Treatment D")
pal <- c("Non-study" = "#000000", "Treatment A" = "#f8766d", "Treatment B" = "#7cae00", "Treatment C" = "#00bfc4", "Treatment D" = "#C3B1E1")
...
# Load data
processed_data <- read.csv("data/processed/processed_data.csv") %>%
  mutate(days_from_trt = floor(as.numeric(difftime(imaging_date, trt_date, units = "days")))) %>%
...
```{r Kaplan-Meier Curve, message=FALSE, warning=FALSE, fig.height=10, fig.width=11.5}
# Load data
processed_data <- read.csv("data/processed/processed_data.csv") %>%
  ...
  mutate(
    ...
    time = ifelse(!is.na(death_date), difftime(death_date, trt_injection_date, units = "days"), NA)
  )
...
ggsurvplot(kmcurve, data = processed_data, xlim = c(0, 63), break.x.by = 7, ylab = "Survival", xlab = "Days",
		   pval = TRUE,
		   risk.table = TRUE,
		   risk.table.title = "",
		   legend.title = "",
...
summary_table <- summary(kmcurve, times = seq(0, 63, 7))
...
Patient derived xenograft named XYZ was thawed and injected on 2024-02-10. The vial was XYZ-GFP-CBG-passage2-2023-08-22-MM-86e6. Total cell count was 80e6 cells with a viability of .712 for a total live cell count of 57e6 cells. 25 mice were injected. Each mouse injected with .2cc/200uL of 1e6 cells.

#### Treatment

##### Treatment A

Treatment A was thawed and injected on 2024-02-23. The vial was 'Treatment A'. Total cell count was Xe6 cells with a viability of X for a total live cell count of Xe6 cells. 5 mice were injected. Each mouse injected with .2cc/200uL of 1e6 cells.

##### Treatment B

Treatment B was thawed and injected on 2024-02-23. The vial was 'Treatment B'. Total cell count was Xe6 cells with a viability of X for a total live cell count of Xe6 cells. 5 mice were injected. Each mouse injected with .2cc/200uL of 1e6 cells.

##### Treatment C

Treatment C was thawed and injected on 2024-02-23. The vial was 'Treatment C'. Total cell count was Xe6 cells with a viability of X for a total live cell count of Xe6 cells. 5 mice were injected. Each mouse injected with .2cc/200uL of 1e6 cells.

##### Treatment D

Treatment D was thawed and injected on 2024-02-23. The vial was 'Treatment D'. Total cell count was Xe6 cells with a viability of X for a total live cell count of Xe6 cells. 5 mice were injected. Each mouse injected with .2cc/200uL of 1e6 cells.
