...
inj_date <- as.Date("na", format = "%m/%d/%Y")

trt_date <- as.Date("na", format = "%m/%d/%Y")

eos_date <- as.Date("na", format = "%m/%d/%Y")

engraftment_imaging_date <- as.Date("na", format = "%m/%d/%Y")

dates <- as.Date(c("na", "na", "na", "na"), format = "%m/%d/%Y")
...
#setwd(dirname(rstudioapi::getSourceEditorContext()$path)) i run this when i execute code locally by chunk
...
# Randomly assign mice to treatment conditions so that each treatment condition has equivalent total_flux between treatments. All of the unassigned remaining mice have the lowest total_flux and are assigned to trt = na aka the expansion group/untreated group. Basically this function runs through 5000 seeds for random group combinations, then performs ANOVAs on the group means for each combination, then returns the seed with the lowest standard deviation of mean total_flux across groups.
...
vertical_lines <- data.frame(
  date = c(inj_date, trt_date, eos_date),
  text = c(
    format(inj_date, format = "%m/%d/%Y"),
    format(trt_date, format = "%m/%d/%Y"),
    format(eos_date, format = "%m/%d/%Y")
  )
)
...
if (plot_type == "Line Graph") {
  for (i in 1:3) {
    p <- p + geom_vline(data = vertical_lines[i, ], aes(xintercept = as.numeric(date), text = text))
  }
}
...
for (i in 1:3) {
  p <- p + geom_vline(data = vertical_lines[i, ], aes(xintercept = as.numeric(date), linetype = "dotted", color = "black", text = text))
}
...
On %m/%d/%Y, # vials of EEE were thawed with a total cell count of #e6 cells and a viability of #%.
There were #e6 cells per 1mL and that was suspended in #mL for a live total cell concentration of #e6 cells.
# mice were injected with #e6 cells each at #uL.
#e6 cells were in #mL from the original #mL suspension.
# eppendorf tubes were used for exactly # injections.
Each eppendorf tube had # injections with #mL of #e6 cells from the original #mL cell suspension and #mL of PBS was added to get a total eppendorf volume of #mL for each eppendorf tube.
...
# Convert 'mass_date' to date format with specified format
raw_mass$mass_date <- as.Date(raw_mass$mass_date, format = "%m-%d-%Y")
