...
read_process_save_csv <- function(file) {
  ...
  # Use the file name as the imaging_date, assuming it's formatted as yyyy-mm-dd
  ...
  # Mutate to add the 'imaging_date' column using the file name and convert to date format
  df <- df %>%
    mutate(imaging_date = ymd(imaging_date))  # Ensure it's recognized as a date
...
high_flux_mice <- processed_data %>%
  filter(imaging_date == most_recent_imaging_date,
         total_flux >= 2e+10,
         is.na(death_date))  # Correct filter to find only alive mice
...
# Check if there are any alive mice (death_date is NA)
