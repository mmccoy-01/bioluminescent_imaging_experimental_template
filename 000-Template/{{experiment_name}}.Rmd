...
number_of_groups <- 5
mice_per_group <- 5
filtered_mice <- setdiff(1:30, c(5, 6, 14, 22, 24))
#engraftment_imaging_date <- "na"
labels <- c("0" = "NA", "1" = "NA", "2" = "NA", "3" = "NA", "4" = "NA", "5" = "NA")
pal <- c("NA" = "#000000", "NA" = "#f8766d", "NA" = "#7cae00", "NA" = "#00bfc4", "NA" = "#C3B1E1", "NA" = "orange")

# Link to organization logo
src <- "https://yt3.googleusercontent.com/ytc/APkrFKbprBkcDrFaaYruHcnwzSgxUJV2N0799D788ar6dQ=s900-c-k-c0x00ffffff-no-rj"
...
processed_data <- processed_data %>%
  mutate(trt_factor = factor(case_when(
    id %in% c(8, 27) ~ 0,
    id %in% c(4, 18, 19, 23, 25) ~ 1,
    id %in% c(6, 7, 13, 17, 20) ~ 2,
    id %in% c(1, 3, 14, 21, 24) ~ 3,
    id %in% c(5, 9, 11, 16, 26) ~ 4,
    id %in% c(2, 10, 12, 15, 22) ~ 5
  ))) %>%
  mutate(trt = factor(case_when(
    id %in% c(8, 27) ~ "Non-study",
    id %in% c(4, 18, 19, 23, 25) ~ "CART38 + Dasatinib",
    id %in% c(6, 7, 13, 17, 20) ~ "CART38 alone",
    id %in% c(1, 3, 14, 21, 24) ~ "Dasatinib alone",
    id %in% c(5, 9, 11, 16, 26) ~ "Untransduced T-cells",
    id %in% c(2, 10, 12, 15, 22) ~ "Untreated"
  )))
...
# Load data
processed_data <- read.csv("data/processed/processed_data.csv") %>%
  mutate(days_from_trt = floor(as.numeric(difftime(imaging_date, as.Date(processed_data$trt_injection_date[1], format = "%Y-%m-%d"), units = "days")))) %>% 
...
```{r Survival by Mouse and by Treatment, message=FALSE, warning=FALSE}
# Read data
processed_data <- read.csv("data/processed/processed_data.csv")
